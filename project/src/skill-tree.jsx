...
// Is the SKILL ITSELF unlocked? All skills are unlocked from start — the
// avatar is a ragdoll at the beginning and should train anything it wants.
function isUnlocked(brains, skillId) {
  if (typeof window !== "undefined" && window.DEMO_UNLOCK_ALL !== false) return true;
...
// All 3 levels are trainable from the start. The ragdoll learns by doing.
function highestTrainableLevel(brains, skillId) {
  if (!isUnlocked(brains, skillId)) return 0;
  if (typeof window !== "undefined" && window.DEMO_UNLOCK_ALL !== false) return 3;
